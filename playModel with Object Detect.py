...
import pyvjoy
from SSDobjectDetect import detectObj
...
while(True):
        ...
        if play:
                ...
                prediction = model.predict([screen.reshape(-1,200,66,3)])     # New RGB models
                # prediction = model.predict([screen.reshape(-1,160,120,1)])    # Old grayscale models
                # print(prediction)
                ...
                vjoy.data.lButtons = handBrake
                vjoy.update()

                # print("FPS: ", round(1/(time.time()-last_time), 1))
                last_time = time.time()
